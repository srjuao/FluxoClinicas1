...
export const SearchReportsModal = ({ clinicId, preselectedPatient, onClose }) => {
  ...
  useEffect(() => {
    ...
    const search = async () => {
      ...
      if (error) {
        toast({ title: 'Erro ao buscar laudos', description: error.message, variant: 'destructive' });
      } else {
        setReports(data);
      }
      setLoading(false);
          ...
          <>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-2">Paciente: {report.patient_name}</p>
                        <div className="flex items-center space-x-4 text-xs text-gray-500">
                          <span>CPF: {report.patient_cpf}</span>
                          <span>Nasc: {new Date(report.patient_birth_date).toLocaleDateString('pt-BR')}</span>
            ...
            {searchTerm.length < 3 && (
              <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">Digite 3 ou mais caracteres para buscar</p>
              </div>
            )}
          </>
        ) : (
          <div className="space-y-6">
            ...
            <div className="glass-effect rounded-xl p-6">
              ...
              <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                <p className="text-sm font-semibold text-blue-900 mb-2">Informações do Paciente</p>
                  <div className="space-y-1 text-sm text-blue-700">
                    <p>Nome: {selectedReport.patient_name}</p>
                    <p>CPF: {selectedReport.patient_cpf}</p>
                    <p>Data de Nascimento: {new Date(selectedReport.patient_birth_date).toLocaleDateString('pt-BR')}</p>
                  </div>
              </div>

              <div>
                <p className="text-sm font-semibold text-gray-700 mb-2">Conteúdo do Laudo:</p>
              ...
              <div className="mt-4 text-xs text-gray-500">
                Criado em: {new Date(selectedReport.created_at).toLocaleString('pt-BR')}
              </div>
            </div>
          </div>
        )}
