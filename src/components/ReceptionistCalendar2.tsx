...
const ReceptionistCalendar = ({ clinicId, doctorId }) => {
  ...
  const getDoctorWorkHours = (date) => {
    ...
    const specific = workHours.find((wh) => wh.specific_date === dateStr);
    ...
    return (
      ...
      >
        <div className="text-xs font-semibold">
          {aptStart.toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
          })}{" "}
          -
          {aptEnd.toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
          })}
        </div>
      ...
      >
        ...
        >
          ...
          <div className="space-y-3">
            <div
              className={`${status.bg} ${status.text} px-3 py-2 rounded-lg text-center font-semibold`}
            >
              {status.label}
            </div>
              ...
              <div className="grid grid-cols-[80px_1fr]">
                ...
                <div>
                  {timeSlots.map((time, idx) => (
                    <div
                      key={time}
                      className="h-12 border-b border-gray-200 p-2 text-xs text-gray-600 text-right pr-3"
                    >
                      {idx % 2 === 0 ? time : ""}
                    </div>
                  ))}
